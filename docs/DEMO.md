...
4. **Live stations** — ask *Is it raining in Hilo right now?* → the Mesonet page opens on the live dashboard of the
   nearest station, IPIF in Hilo (`/mesonet?viewer=live&station=0281&view=dashboard`). Tip: the embedded HCDP
   dashboard shows its licence box on a browser's first visit — tick it once before the demo. Change the station or view in the picker: the URL
